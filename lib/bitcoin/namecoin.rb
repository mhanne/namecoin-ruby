...
module Bitcoin::Namecoin
  ...
  def self.load
    ...
    Bitcoin::Blockchain::Backends::Base.class_eval { include Blockchain::Backend }
    Bitcoin::Blockchain::Models.class_eval { include Blockchain::Models }
  end
        ...
        base.class_eval do
          ...
          def store_name(script, txout_id)
            if script.type == :name_new
              ...
              @db[:names].where(txout_id: name_new[:txout_id], name: nil)
                .update(name: script.get_namecoin_name.to_s.to_sequel_blob)
              @db[:names].insert(txout_id: txout_id, hash: name_hash,
              ...
              log.debug { "#{script.type}: #{script.get_namecoin_name}" }
              @db[:names].insert(txout_id: txout_id,
                name: script.get_namecoin_name.to_s.to_sequel_blob,
                value: script.get_namecoin_value.to_s.to_sequel_blob)
            end
          end
          ...
          def wrap_name(data)
            ...
            Bitcoin::Blockchain::Models::Name.new(self, data)
          end
