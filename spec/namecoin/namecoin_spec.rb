   ...
   { name: :archive, db: 'sqlite:/' },
    ...
    describe "Namecoin (#{configuration[:name]} store)" do
      ...
      before do
        ...
        class Bitcoin::Blockchain::Validation::Block; def min_timestamp; true; end; end
        Bitcoin.network[:proof_of_work_limit] = Bitcoin.encode_compact_bits("ff"*32)
        [:name_new, :name_firstupdate, :name_update].each {|type|
          Bitcoin::Blockchain::Backends::Archive::SCRIPT_TYPES << type }
        @store = Bitcoin::Blockchain.create_store(configuration[:name], configuration)
        @store.reset
      ...
      it "should store names" do
        ...
        @store.db[:names][hash: Bitcoin.hash160(@rand + "test".hth)].should_not == nil
